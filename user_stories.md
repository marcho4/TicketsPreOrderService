...
(Авторизованное состояние -> Выбор матча -> Матч еще не прошел -> «Купить билет» -> выбор места -> 
...
**- Я как пользователь должен иметь возможность оплатить свой билет (важно)** <br/>
(Заказ оформлен -> перенаправление на платежную систему -> оплата -> страница с сообщением об успешной оплате)

-----

**- Я как пользователь должен иметь возможность вернуть средства за свой билет (важно)** <br/>  
  (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Получение билета по (условно почте) -> Личный кабинет -> Мои предзаказы -> Переход на окно с предзаказами -> Отменить предзаказ (вернуть деньги за билет) -> Матч еще не прошел -> Предоставление необходимой информации для возврата -> Возврат в личный кабинет)
  
-----

**- Я как пользователь должен иметь возможность увидеть свои предзаказы (важно)** <br/>  
  (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Личный кабинет -> Мои предзаказы -> Переход на окно с предзаказами -> Кнопка возврата в личный кабинет)
  
-----

**- Я как пользователь должен иметь возможность получить уведомление об успешном предзаказе (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Получение уведомления о покупке (предзаказе))  
  
-----

**- Я как пользователь должен иметь возможность получить уведомление об успешной отмене (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Личный кабинет -> Мои предзаказы -> Матч еще не прошел -> Отмена предзаказа -> Получение уведомления об успешной отмене)
  
-----

**- Я как пользователь должен иметь возможность поставить уведомление на появление билета в наличии, которого сейчас нет (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Личный кабинет -> Выбор мероприятия -> Выбор отсутствующего билета -> Установка уведомления на наличие)
  
-----

**- Я как пользователь должен иметь возможность ранжировать места по ценовой категории (Bronze, Platinum, Standard, Gold) (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Выбор мероприятия -> Просмотр мест -> Сортировка по ценовой категории)
  
-----

**- Я как пользователь должен иметь возможность посмотреть контакты организатора для связи (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Личный кабинет -> Выбор мероприятия -> Просмотр контактов организатора)
  
-----

**- Я как пользователь должен иметь возможность оставить отзыв о матче/сервисе (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Личный кабинет -> Мои предзаказы -> Выбор мероприятия -> Оставить отзыв)
...
(кнопка "поддержка"(она будет в окне с обычной авторизацией) -> кнопка "отправка заявления о желании стать организатором" -> заполнение заявления: ФИО, электронная почта для обратной связи, ИНН организации -> ожидаете ответа -> после того как ваше заявление одобрят, вам на почту придет письмо с вашим логином и паролем -> переход в авторизованное состояние, в качестве организатора (другой интерфейс))
...
**- Я как организатор матчей должен иметь возможность загрузить данные о матче (дата, место проведения - обязательные, доп. информация, кол-во мест, мин и макс цена - необязательные) (важно)** <br/>
...
(Матч создан -> Матч еще не прошел -> кнопка "изменить сроки продажи билетов" -> выбор нового интервала дат (по умолчанию от времени создания до время начала матча) -> кнопка "подтвердить")

-----

**- Я как организатор должен иметь возможность поставить лимит на кол-во билетов в одни руки (неважно)** <br/>
(Матч создан -> Матч еще не прошел -> кнопка "изменить лимит на покупку билетов в одни руки" -> выбор нового лимита (по умолчанию 5) -> кнопка "подтвердить")

-----

**- Я как организатор матчей должен иметь возможность изменить данные о матче (важно)** <br/>
(Матч создан -> Матч еще не прошел -> кнопка "изменить данные о матче" -> ввод новых обязательных данных -> ввод новых второстепенных данных -> кнопка "изменить")

-----

**- Я как организатор матчей должен иметь возможность изменить данные о билетах на матч (важно)** <br/>
(Матч создан, хотя бы 1 билет добавлен -> Матч еще не прошел -> кнопка "изменить билеты" ->
...
(Матч создан -> Матч еще не прошел -> кнопка "оповестить покупателей" -> ввод сообщения об изменениях -> кнопка "отправить" -> все пользователи купившие билеты на матч получат это сообщение -> показ подтверждения отправки сообщений от бэкэнда)

-----

**- Я как организатор матчей должен иметь возможность удалить данные о билетах на матч (важно)** <br/>
(Матч создан, хотя бы 1 билет добавлен -> Матч еще не прошел -> кнопка "удалить билеты" ->
...
(Авторизованное состояние, в качестве организатора -> кнопка "статистика" -> отображение количества проданных билетов и общей выручки -> возможность фильтрации по времени, матчам, и т.д.)
...
(Авторизованное состояние, в качестве организатора -> Выбор мероприятия -> кнопка "экспорт данных" -> выбор формата файла (csv, docx) -> выбор типа данных (все билеты, только проданные) -> кнопка "сохранить файл")

-----

**- Я как организатор должен иметь возможность загрузить карту стадиона (второстепенно)**  
(Авторизованное состояние, в качестве организатора -> Выбор мероприятия -> Матч еще не прошел -> кнопка -> "загрузить карту стадиона" -> выбор файла с картой (например, изображение или схема) -> кнопка "сохранить карту" -> карта отображается в интерфейсе)


-----

**- Я как организатор должен иметь возможность изменить данные для входа(важно)**  
(Авторизованное состояние, в качестве организатора -> личный кабинет -> Изменить данные -> ввод данных -> подтверждение -> Личный кабинет)


-----
**Я как админ должен иметь возможность подтвердить регистрацию (важно)**
(ручная проверка)
