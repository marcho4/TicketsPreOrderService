...
**- Я как пользователь должен иметь возможность вернуть средства за свой билет (важно)** <br/>  
  (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Получение билета по (условно почте) -> Личный кабинет -> Мои предзаказы -> Переход на окно с предзаказами -> Отменить предзаказ (вернуть деньги за билет) -> Предоставление необходимой информации для возврата -> Возврат в личный кабинет)
...
**- Я как пользователь должен иметь возможность получить уведомление об успешной отмене (второстепенно)** <br/>  
  (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Личный кабинет -> Мои предзаказы -> Отмена предзаказа -> Получение уведомления об успешной отмене)
...
(Авторизованное состояние, в качестве организатора -> Выбор мероприятиякнопка -> "загрузить карту стадиона" -> выбор файла с картой (например, изображение или схема) -> кнопка "сохранить карту" -> карта отображается в интерфейсе)
