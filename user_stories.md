...
**- Я как пользователь должен иметь возможность вернуть средства за свой билет (важно)** <br/>
   (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Получение билета по (условно почте) -> Личный кабинет -> Мои предзаказы -> 
   Переход на окно с предзаками -> Отменить предказ (вернуть деньги за билет) -> Предоставление необходимой информации для возврата -> Возврат в личный кабинет)
**- Я как пользователь должен иметь возможность увидеть свои предзаказы (важно)** <br/>
   (Регистрация -> Авторизация -> Привязка банка (для совершения транзакций) -> Оплата билета -> Личный кабинет -> Мои предзаказы -> 
   Переход на окно с предзаками -> Кнопка возврата в личный кабинет)
